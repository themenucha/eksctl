			...
			Context("create test workloads", func() {
				...
				It("should be able to run pods with an iamserviceaccount", func() {
					...
					for _, pod := range pods.Items {
						...
						Expect(so.AssumedRoleUser.Arn).To(HavePrefix("arn:aws:sts::"))
						Expect(so.AssumedRoleUser.Arn).To(ContainSubstring(":assumed-role/eksctl-"+clusterName+"-addon-iamserviceaccount-"))
						Expect(so.AssumedRoleUser.Arn).To(HaveSuffix("/integration-test"))

						Expect(so.Audience).To(Equal("sts.amazonaws.com"))

						Expect(so.Provider).To(HavePrefix("arn:aws:iam::"))
						Expect(so.Provider).To(ContainSubstring(":oidc-provider/oidc.eks."+region+".amazonaws.com/id/"))
